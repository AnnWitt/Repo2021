...
import itneoTrener.varargs.model.Car;
import itneoTrener.varargs.model.Kolor;
...
public class VarArgsPrzyklady {
    ...
    //* c) Stwórz filter słów, które nie są akceptowalne społecznie zamień je na *****. Wylicz wszystkie
    // nieakceptowalne społecznie słowa w tekście.
    //--------------------

    private static String filterczyZawieraSłowoNiecenzurowane(String tekst, String ...nieCenzurowaneSlowa){
        String[] words=tekst.split("[ ]");
        String wynik="";
        for (String word: words) {
            if (czyZawieraSłowo(word,nieCenzurowaneSlowa)){
                //kolejna funkcja
                wynik=wynik+" " + zammienNaGwiazdki(word);
            } else {
                wynik= wynik+" "+ word;
            }
        }
    ...
    private static String zammienNaGwiazdki(String word) {
        String wynik="";
